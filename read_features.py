...
def topological_get_layer_head(features, layer, head, topological_feature_names):
    ...
    try:
        ...
        errmsg = 'Check topological_feature_names length, it is uncompatiable with the df.shape (redundant w?)'
...
def load_data(dataset_name, model_name, feature_dir="./features",heads="all", layers=12,
    ...
    pbar = tqdm.tqdm(total=len(all_features_dict)*12*layers, desc="Loading {} features...".format(dataset_name), position=0, leave=True)
    for feature_type, read_args in all_features_dict.items():
        function_, feature_names, path = read_args
        path = os.path.join(feature_dir, path + '.npy')
        features_table = np.load(path)
